...
public class SupplyModuleService {
    private final BusinessLayer.Controller controller;

    public SupplyModuleService(){
        controller=new BusinessLayer.Controller();
    }

    public Response<BusinessLayer.Supplier> AddSupplier(Integer companyNumber, String name, String bankNumber, List<ContactPerson> contactPeople, String orderingCP){
        try{
            return new IsValue<BusinessLayer.Supplier>(controller.AddSupplier(name, companyNumber, bankNumber, contactPeople,orderingCP),"Supplier added");
        }
        catch (Exception e){
